...
use redeye_tracer::api::handlers::{ingest_handler, traces_handler, audit_handler, compliance_metrics_handler};
...
async fn setup_test_environment() -> TestEnv {
    ...
    let clickhouse_node = clickhouse_image.start().await.expect("Failed to start ClickHouse container");
    let ch_port = clickhouse_node.get_host_port_ipv4(8123).await.expect("Failed to get Clickhouse port");
    ...
    assert!(db_ready, "Fatal error: Clickhouse testcontainer failed network binding");
    ...
    let raw_ddl = std::fs::read_to_string("../infra/clickhouse/init.sql").expect("Missing init.sql");
    for query in raw_ddl.split(';') {
        let q = query.trim();
        if !q.is_empty() {
             client.post(&clickhouse_url)
                .body(q.to_string())
                .send()
                .await
    ...
    let app = axum::Router::new()
        ...
        .route("/v1/compliance/metrics", axum::routing::get(compliance_metrics_handler))
...
async fn test_ingest_and_assert_clickhouse() {
    ...
    assert_eq!(res_ingest.status(), StatusCode::OK, "Failed to ingest telemetry logs");
    ...
    let raw_db_req = client.post(&env.clickhouse_url)
        .body(query.to_string())
        .send()
        .await
        .expect("Failed to execute external raw query against ClickHouse Container HTTP wrapper");

    assert!(raw_db_req.status().is_success(), "Raw DB query failed execution");
    ...
    assert_eq!(count, 1, "Clickhouse failed to persist and return exact ingested metric mapping traces");
}
