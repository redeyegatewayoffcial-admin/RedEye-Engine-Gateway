...
function StatCard({ title, value, icon: Icon, colorClass, subtitle }: { title: string, value: string | number, icon: any, colorClass: string, subtitle?: string }) {
  return (
    <div className="glass-panel p-4 sm:p-6 flex items-start space-x-3 sm:space-x-4 hover:-translate-y-1 transition-transform duration-300 w-full">
      <div className={`p-2 sm:p-3 rounded-lg bg-nexus-panel flex-shrink-0 ${colorClass}`}>
        <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-xs sm:text-sm font-medium text-slate-400 truncate">{title}</p>
        <h3 className="text-xl sm:text-2xl font-bold mt-1 tracking-tight text-white truncate">{value}</h3>
        {subtitle && <p className="text-[10px] sm:text-xs text-emerald-400 mt-1 font-medium truncate">{subtitle}</p>}
      </div>
    </div>
  );
}
...
function App() {
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col p-4 sm:p-6 lg:p-8 overflow-x-hidden bg-nexus-dark">
      ...
      <header className="mb-6 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent pb-1">
            NexusAI Gateway
          </h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">Enterprise Telemetry & Security</p>
        </div>
        <div className="flex items-center space-x-2 glass-panel px-3 py-1.5 sm:px-4 sm:py-2 rounded-full self-start sm:self-auto w-fit">
          <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${error ? 'bg-red-500' : 'bg-emerald-500 animate-pulse'}`} />
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
          <StatCard title="Total Traffic" value={metrics?.total_requests || "0"} icon={Activity} colorClass="text-blue-400 ring-1 ring-blue-400/20" />
          <StatCard title="Avg Latency" value={`${Math.round(metrics?.avg_latency_ms || 0)} ms`} icon={Zap} colorClass="text-emerald-400 ring-1 ring-emerald-400/20" />
          <StatCard title="Tokens Processed" value={metrics?.total_tokens || "0"} icon={Cpu} colorClass="text-purple-400 ring-1 ring-purple-400/20" />
          <StatCard title="Threats Blocked" value={metrics?.rate_limited_requests || "0"} icon={ShieldAlert} colorClass="text-orange-400 ring-1 ring-orange-400/20" />
          <div className="sm:col-span-2 lg:col-span-1 xl:col-span-1">
            <StatCard title="API Cost Saved" value={`$${calculateSavedCost()}`} icon={DollarSign} colorClass="text-green-400 ring-1 ring-green-400/20" subtitle="Prevented by Gateway" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="glass-panel p-4 sm:p-6 lg:col-span-2">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-4">Live Traffic Overview</h2>
            <div className="h-[200px] sm:h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Line type="monotone" dataKey="requests" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="glass-panel p-4 sm:p-6 lg:col-span-1">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-4">Model Distribution</h2>
            <div className="h-[200px] sm:h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="glass-panel p-4 sm:p-6 lg:col-span-1">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-4">Latency Histogram</h2>
            <div className="h-[200px] sm:h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="glass-panel p-4 sm:p-6 lg:col-span-2 flex flex-col overflow-hidden">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-4">Live Request Audit Log</h2>
                ...
                <tbody className="text-xs sm:text-sm">
                  {mockAuditLogs.map((log, i) => (
                    <tr key={i} className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-2 sm:py-3 text-blue-400 whitespace-nowrap pr-4">{log.tenant}</td>
                      ...
                      <td className="py-2 sm:py-3 whitespace-nowrap pr-4">
                        <span className={`px-2 py-1 rounded text-[10px] sm:text-xs font-semibold ${log.status === 200 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                          {log.status}
                        </span>
                      </td>
