...
use crate::{
    domain::models::{
        ComplianceAuditRecord, CompliancePolicy, OpaInput, OpaRequestPayload, ResidencyRule,
    },
...
pub async fn security_guard_middleware(
    ...
    if let Ok(payload) = serde_json::from_slice::<Value>(&bytes) {
        ...
        if let Some(prompt) = payload.get("prompt").and_then(|v| v.as_str()) {
            if contains_prompt_injection(prompt) {
                ...
                policy_result = false;
                block_reason =
                    Some("Security Violation: Malicious prompt pattern detected.".to_string());
            }
        }
                ...
                Ok(result) => {
                    if !result.allow {
                        ...
                        block_reason = Some(
                            result
                                .block_reason
                                .unwrap_or_else(|| "Blocked by organizational policy".to_string()),
                        );
                    }
                }
                Err(_e) => {
                    if state.compliance_policy.fail_closed {
                        ...
                        policy_result = false;
                        block_reason =
                            Some("Security System Unavailable (Fail-Closed active)".to_string());
                    }
                }
            }
        }
    ...
    state
        .clickhouse
        .log_audit_event(ComplianceAuditRecord {
            trace_id: trace_id.clone(),
            tenant_id: tenant_id.clone(),
            timestamp,
            policy_result,
