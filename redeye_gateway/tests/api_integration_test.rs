...
use redeye_gateway::infrastructure::cache_client::proto::{
    CacheRequest, CacheResponse, StoreAck, StoreRequest,
};
...
impl CacheService for MockCacheMiss {
    async fn lookup_cache(
        ...
        Ok(tonic::Response::new(CacheResponse {
            hit: false,
            content: String::new(),
        }))
    }
...
async fn test_grpc_cache_miss_proceeds_to_llm() {
    ...
    use redeye_gateway::infrastructure::cache_client::{lookup_cache, CacheGrpcClient};
    ...
    assert_eq!(
        result, None,
        "Cache miss must return None so the gateway proceeds to the LLM"
    );
}
...
async fn test_grpc_cache_unavailable_is_failopen() {
    ...
    assert_eq!(
        result, None,
        "Code::Unavailable must cause fail-open (None), not a panic"
    );
}
...
async fn test_grpc_store_succeeds_within_timeout() {
    use redeye_gateway::domain::models::TraceContext;
    use redeye_gateway::infrastructure::cache_client::{store_in_cache, CacheGrpcClient};
...
async fn test_happy_path_chat_completion() {
    ...
    assert_eq!(
        ...
        "Hello from mock wiremock OpenAI!"
    );
}
...
async fn test_rate_limit_exhaustion() {
    ...
    assert_eq!(
        successes, rate_max,
        "Exactly rate_limit_max requests should succeed"
    );
...
async fn test_hot_swap_failover() {
    ...
    assert_eq!(
        response.status(),
        StatusCode::OK,
        "Expected Hot-Swap to recover with 200 OK"
    );
