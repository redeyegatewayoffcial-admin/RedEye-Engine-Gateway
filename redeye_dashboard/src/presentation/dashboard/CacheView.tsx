  ...
  if (isLoading) {
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="h-48 bg-white/80 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/80 rounded shadow-sm"></div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-50">Semantic Cache</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="glass-panel bg-white/80 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/80 p-5 lg:col-span-2 shadow-sm backdrop-blur-md dark:shadow-none transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Hit / Miss Overview</h2>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Hit Ratio</p>
              <p className="text-3xl font-semibold text-emerald-600 dark:text-emerald-400">
                {(stats.hit_ratio * 100).toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Miss Ratio</p>
              <p className="text-3xl font-semibold text-rose-600 dark:text-rose-400">
                {(stats.miss_ratio * 100).toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Total Lookups</p>
        ...
        <div className="glass-panel bg-white/80 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/80 p-5 lg:col-span-1 shadow-sm backdrop-blur-md dark:shadow-none transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2">Cache Insights</h2>
