  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-rose-400 via-amber-300 to-indigo-400 bg-clip-text text-transparent pb-1">
      ...
      {error && !data && (
        <div className="glass-panel border-rose-500/20 bg-rose-500/5 p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-slate-300">Failed to connect to security endpoint. Showing stale or zeroed data.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6">
        ...
        <div className="glass-panel bg-slate-900/40 border border-slate-800/80 p-4 sm:p-6 lg:col-span-2">
          <h2 className="text-lg sm:text-xl font-bold text-slate-100 mb-6 flex items-center gap-2">
            Blocked Threats / Day
            <span className="text-[10px] text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded font-mono uppercase tracking-tighter">7-day</span>
          </h2>
          <div className="h-[280px] w-full min-h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data?.daily_blocks ?? []} barGap={4}>
                ...
                <Bar dataKey="loops" name="Loop Blocks" fill="#f43f5e" radius={[4, 4, 0, 0]} animationDuration={1200} />
                <Bar dataKey="burn_rate" name="Burn Rate Blocks" fill="#f59e0b" radius={[4, 4, 0, 0]} animationDuration={1200} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-panel bg-slate-900/40 border border-slate-800/80 p-4 sm:p-6 lg:col-span-3 flex flex-col overflow-hidden">
          <h2 className="text-lg sm:text-xl font-bold text-slate-100 mb-4 flex items-center justify-between">
            Live Threat Feed
            <span className="text-[10px] text-rose-400 bg-rose-500/10 px-1.5 py-0.5 rounded font-mono uppercase tracking-widest animate-pulse">Live</span>
          </h2>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {(data?.recent_alerts ?? []).map((alert, i) => (
                  ...
                  >
                    <td className="py-2.5 px-3 font-mono text-xs text-slate-400">{formatTime(alert.timestamp)}</td>
                    <td className="py-2.5 px-3 font-mono text-xs text-indigo-300">{alert.session_id}</td>
                    <td className="py-2.5 px-3 text-slate-200">{alert.reason}</td>
