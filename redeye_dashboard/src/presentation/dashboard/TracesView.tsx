    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-50 font-display">Trace Explorer</h1>
          <p className="text-sm text-slate-400 mt-1">Inspect request flows, tenant context, and policy outcomes.</p>
        </div>
        <div className="hidden sm:flex items-center gap-2 text-slate-400 text-xs font-medium bg-slate-900/50 px-3 py-1.5 rounded-full border border-slate-800">
          <Activity className={`w-4 h-4 ${error ? 'text-rose-400' : 'text-emerald-400'}`} />
      ...
      <div className="glass-panel bg-slate-900/40 border border-slate-800/80 p-5 flex flex-col overflow-hidden">
        ...
        {error ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-slate-400 max-w-xs">Failed to fetch live traces. Ensure the gateway service is reachable.</p>
          </div>
        ) : (
              ...
              <tbody className="text-xs sm:text-sm">
                {isLoading && !traces ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i} className="border-b border-slate-900/70 animate-pulse">
                      <td className="py-4 pl-2"><div className="h-3 w-24 bg-slate-800 rounded"></div></td>
                      <td className="py-4"><div className="h-3 w-16 bg-slate-800 rounded"></div></td>
                      <td className="py-4"><div className="h-3 w-12 bg-slate-800 rounded"></div></td>
                      <td className="py-4"><div className="h-3 w-8 bg-slate-800 rounded"></div></td>
                      <td className="py-4"><div className="h-5 w-16 bg-slate-800 rounded"></div></td>
                      <td className="py-4 pr-2"><div className="h-3 w-10 bg-slate-800 rounded"></div></td>
                    </tr>
                  ))
                ) : (
                  traces?.map((trace) => (
                    <tr key={trace.traceId} className="border-b border-slate-900/70 hover:bg-slate-900/70 transition-colors group">
                      <td className="py-4 text-slate-200 whitespace-nowrap pr-4 font-mono text-[11px] pl-2 group-hover:text-indigo-300 transition-colors">
                        {trace.traceId}
                      </td>
                      <td className="py-4 text-slate-400 whitespace-nowrap pr-4">{trace.tenantId}</td>
                      <td className="py-4 text-slate-300 whitespace-nowrap pr-4">{trace.model}</td>
                      <td className="py-4 text-slate-500 whitespace-nowrap pr-4 font-mono text-xs">{trace.tokens}</td>
                      ...
                      <td className="py-4 text-slate-300 whitespace-nowrap pr-2 tabular-nums font-medium text-right sm:text-left">
                        {trace.latency}
                      </td>
                    </tr>
                  ))
